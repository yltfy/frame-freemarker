...
public class Page {
	...
	private static final String PAGE	= "page/PAGE.txt";	
	
	public Page(){
		try {
			this.showCount = Integer.parseInt(Tools.readTxtFile(PAGE));
		} catch (Exception e) {
			this.showCount = 15;
		}
	}
	...
	public String getPageStr() {
		...
		if(totalResult>0){
			...
			if(currentPage==1){
				...
				sb.append("	<li><input type=\"number\" value=\"\" id=\"toGoPage\" style=\"width:50px;text-align:center;line-height:28px;color:#000;float:left\" placeholder=\"页码\"/></li>\n");
				sb.append("	<li style=\"cursor:pointer;\"><a onclick=\"toTZ();\"  class=\"btn\" style=\"background:#87b87f;color:#fff;\">跳转</a></li>\n");
			...
			sb.append("	<li><select title='显示条数' style=\"width:55px;height:30px;float:left;\" onchange=\"changeCount(this.value)\">\n");
			sb.append("	<option value='"+showCount+"' style='background:#2b82cc;'>"+showCount+"</option>\n");
			...
			sb.append("		url = url + \"&" +(entityOrField?"showCount":"page.showCount")+"="+showCount+"\";\n");			
			...
			sb.append("			}else{\n");
			sb.append("				url += \"&currentPage1&"+(entityOrField?"currentPage":"page.currentPage")+"=\";\n");
			sb.append("			}\n");
			sb.append("		}else{url += \"?"+(entityOrField?"currentPage":"page.currentPage")+"=\"+1;}\n");
